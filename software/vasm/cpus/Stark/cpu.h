...
#define OP_D8  0x1001
#define OP_D10 0x1002
#define OP_D14 0x1003
#define OP_D16 0x1004
#define OP_D32 0x1005
#define OP_D64 0x1006
#define OP_F32 0x1007
#define OP_F64 0x1008
...
typedef struct {
  ...
  int bsereg;
  int ndxreg;  	/* only for d[Rm+Rn] load/store addressing mode */
  int scale;
  struct mnemonic* mne;
} operand;
...
#define CPU_TYPE_LB650        (0x1000000ULL)
#define CPU_TYPE_STARK        (0x2000000ULL)
...
#undef  PPC
#define LB650		CPU_TYPE_LB650
#define STARK		CPU_TYPE_STARK
...
#define OP(x) ((((uint32_t)(x)) & 0x3f) << 0)
#define OPTO(x,to) (OP (x) | ((((uint32_t)(to)) & 0x1f) << 21))
#define OPL(x,l) (OP (x) | ((((uint32_t)(l)) & 1) << 21))
#define A(op, xop, rc) \
  (OP (op) | ((((uint32_t)(xop)) & 0x1f) << 1) | (((uint32_t)(rc)) & 1))
#define B(op, aa, lk) (OP (op) | ((((uint32_t)(aa)) & 1) << 1) | ((lk) & 7)<<6)
#define BBO(op, bo, aa, lk) (B ((op), (aa), (lk)) | ((((uint32_t)(bo)) & 0xf) << 22))
#define BBOCB(op, bo, cb, aa, lk) \
  (BBO ((op), (bo), (aa), (lk)) | ((((uint32_t)(cb)) & 0x7) << 17))
...
#define X(op, xop) (OP (op) | ((((uint32_t)(xop)) & 0x3f) << 26))
#define XRC(op, xop, rc) (X ((op), (xop)) | ((rc) & 1) << 26)
...
#define XO(op, xop, oe, rc) \
  (OP (op) | ((((uint32_t)(xop)) & 0x3f) << 26) | \
   ((((uint32_t)(oe)) & 1) << 10) | (((uint32_t)(rc)) & 1) << 26)
#define XS(op, xop, rc) \
...
int is_uint10(uint64_t value);
int is_int10(int64_t value);
int is_uint16(uint64_t value);
int is_int16(int64_t value);
