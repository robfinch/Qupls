...
start:
	ldi t0,-1
# Setup page table so some IO may be accessed
	ldi a0,pgtbl								# level 0 table
	sto a0,PTBR
	ldi	a1,0										
.clrpgtbl:
	sto	r0,pgtbl[a1]
	add	a1,a1,8
	bltu a1,8,.clrpgtbl				# number of entries to clear *8

	ldi a0,0x83000FFFFFFFFEDF	# LEDS
	sto	a0,pgtbl+0x1EDF*8
	ldi a0,0x83000FFFFFFFFEC0	# text screen
	sto	a0,pgtbl+0x1EC0*8
	ldi a0,0x83000FFFFFFFFEDC	# keyboard
	sto	a0,pgtbl+0x1EDC*8
	ldi a0,0x83000FFFFFFFFEE1	# random
	sto	a0,pgtbl+0x1EE1*8

# Get the previous stores to complete, otherwise when a TLB miss occurs a page
# fault will occur, and we're not ready for one yet.
	nop
	nop
	sync
	stt t0,leds
